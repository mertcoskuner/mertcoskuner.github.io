...
const Expertise: React.FC<ExpertiseProps> = ({ mode }) => {
  ...
  return (
    <div className="container" id="expertise">
      <h1 style={{fontSize: '2.5rem', marginBottom: '2.5rem', fontWeight: 700, color: colors.title}}>Expertise</h1>
      <div className="expertise-flex" style={{display: 'flex', gap: '3rem', justifyContent: 'center', flexWrap: 'wrap'}}>
        {expertiseData.map((exp, idx) => (
          <div key={idx} className="expertise-card" style={{flex: '1 1 320px', minWidth: 320, maxWidth: 420, background: colors.cardBg, borderRadius: 18, padding: '2.5rem 2rem', margin: '0 0.5rem', display: 'flex', flexDirection: 'column', alignItems: 'flex-start'}}>
            ...
            <div style={{display: 'flex', flexWrap: 'wrap', gap: '0.7em'}}>
              {exp.stack.map((label, i) => (
                <Chip key={i} className='chip' label={label} style={{fontSize: '1.08em', background: colors.chipBg, color: colors.chipColor, fontWeight: 500, padding: '0.5em 1.1em'}} />
              ))}
            </div>
          </div>
        ))}
